...
{
    @Query("SELECT r FROM Reservation r WHERE r.room.getId() = ?1")
    Iterable<Reservation> findByRoomId(int roomId);

    @Query("SELECT r FROM Reservation r WHERE r.owner.getId() = ?1")
    Iterable<Reservation> findByOwnerId(int ownerId);

    @Query("SELECT r FROM Reservation r WHERE r.owner.getId() = ?1 AND r.status = ?2")
